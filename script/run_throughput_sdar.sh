...
# ============================================================
# SDAR throughput benchmark — MultiBD (naive)
...
# ============================================================
# Model paths (math / code)
# ============================================================

SDAR_MATH_MODEL="${SDAR_MATH_MODEL:-/root/data/ckpts/JetLM/SDAR-8B-Chat-b32}"
SDAR_CODE_MODEL="${SDAR_CODE_MODEL:-/root/data/ckpts/JetLM/SDAR-8B-Chat-b32}"
...
CODE_TOKEN_STABILITY_THRESHOLDS=(0.0)
...
# --- Output ---
OUTPUT_BASE="${OUTPUT_BASE:-benchmark_results/throughput_sdar}"
RUN_ID="${RUN_ID:-run_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_BASE}/${RUN_ID}"
LOG_DIR="${LOG_DIR:-logs/throughput_sdar/${RUN_ID}}"
...
# ============================================================
# Build experiment list (paired: model→dataset)
# ============================================================
# Each entry: model_path|dataset|batch|block|buf|ath|sth|acth|rth|stth|max_tok
EXPERIMENTS=()

SDAR_MATH_DATASETS=(gsm8k_sdar math500_sdar)
SDAR_CODE_DATASETS=(humaneval_plus_sdar mbpp_sanitized_sdar)

for mp in "${SDAR_MATH_MODEL}"; do
  for ds in "${SDAR_MATH_DATASETS[@]}"; do
...
for mp in "${SDAR_CODE_MODEL}"; do
  for ds in "${SDAR_CODE_DATASETS[@]}"; do
    for bat in "${BATCH_SIZES[@]}"; do
      for bs in "${BLOCK_SIZES[@]}"; do
        for buf in "${BUFFER_SIZES[@]}"; do
...
# ============================================================
echo "================================================================"
echo "SDAR MultiBD Throughput Benchmark"
echo "================================================================"
...
echo "================================================================"
echo "SDAR finished: ${run_count} run, ${skip_count} skipped, ${fail_count} failed (of ${#EXPERIMENTS[@]} total)"
