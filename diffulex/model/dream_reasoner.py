...
class DreamReasonerAttention(nn.Module):
    """DreamReasoner attention, matching the checkpoint's q/k per-head RMSNorm."""
...
class DreamReasonerMLP(nn.Module):
    def __init__(self, config: DreamConfig) -> None:
        super().__init__()
...
class DreamReasonerDecoderLayer(nn.Module):
    def __init__(self, config: DreamConfig) -> None:
        super().__init__()
        self.self_attn = DreamReasonerAttention(config)
        self.mlp = DreamReasonerMLP(config)
...
class DreamReasonerModel(nn.Module):
    def __init__(self, config: DreamConfig) -> None:
        super().__init__()
        self.embed_tokens = VocabParallelEmbedding(config.vocab_size, config.hidden_size)
        self.layers = nn.ModuleList([DreamReasonerDecoderLayer(config) for _ in range(config.num_hidden_layers)])
...
@AutoModelForDiffusionLM.register("dream_reasoner")
class DreamReasonerForDiffusionLM(nn.Module):
    """DreamReasoner-8B with the local Diffulex multi_bd runner."""

    packed_modules_mapping = {}

    def __init__(self, config: DreamConfig) -> None:
        super().__init__()
        self.model = DreamReasonerModel(config)
