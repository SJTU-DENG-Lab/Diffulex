...
class TokenMergeModelRunnerTemplate(MultiBlockModelRunnerTemplate):
    ...
    def _init_token_merge_metadata(
        ...
        if num_tokens == 0:
            attn_metadata.init_token_merge(mask_token_id=self.config.mask_token_id, enabled=False)
            return
    ...
    def run_multi_block(
        ...
        return self._run_token_merge_subgroup(reqs)
    ...
    def run_model_multi_block(
        ...
        with record_function("diffulex.token_merge.model_forward"):
            ...
            if attn_metadata.has_prefill:
                if full_runner.can_run_prefill(attn_metadata, int(input_ids.size(0))):
                    with record_function("diffulex.token_merge.full_static_prefill"):
                        return full_runner.run_prefill(input_ids, positions, attn_metadata)
                with record_function("diffulex.token_merge.eager_prefill"):
                    return self.model.compute_logits(self.model(input_ids, positions))

            if not full_runner.can_run_decode(input_ids):
                with record_function("diffulex.token_merge.eager_decode"):
                    return self.model.compute_logits(self.model(input_ids, positions))

            with record_function("diffulex.token_merge.full_static_decode"):
    ...
    @torch.inference_mode()
    def capture_cudagraph_token_merge(self: TokenMergeModelRunnerTemplate):
        set_warming_up(True)
        config = self.config
    ...
    def _run_token_merge_subgroup(
        self: TokenMergeModelRunnerTemplate,
        reqs: list[TokenMergeReqTemplate],
    ):
        with record_function("diffulex.token_merge.filter_local_reqs"):
            local_reqs = self.filter_local_reqs(reqs)
        if not local_reqs:
            with record_function("diffulex.token_merge.gather_dp_sample_output"):
                return self.gather_dp_sample_output(None)

        with record_function("diffulex.token_merge.prepare_chunked_prefill"):
            input_ids, positions = self.prepare_chunked_prefill_token_merge(local_reqs)
        with record_function("diffulex.token_merge.prepare_sample"):
        ...
        with record_function("diffulex.token_merge.sampler"):
            sample_output = self.sampler(local_reqs, logits, temperatures) if self.is_model_parallel_root else None
        with record_function("diffulex.token_merge.reset_attn_metadata"):
            self.reset_attn_metadata()
        with record_function("diffulex.token_merge.gather_dp_sample_output"):
