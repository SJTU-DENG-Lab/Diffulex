...
@dataclass
class MultiBlockAttnMetaDataMixin(AttnMetaDataBase):
    def init_multi_block(
        ...
        if self.page_size not in supported_sizes:
            raise ValueError(f"page_size must be one of {supported_sizes}, got {self.page_size}")
        if self.block_size not in supported_sizes:
            raise ValueError(f"block_size must be one of {supported_sizes}, got {self.block_size}")
        if self.block_size > self.page_size:
            raise ValueError(f"block_size {self.block_size} must be <= page_size {self.page_size}")
