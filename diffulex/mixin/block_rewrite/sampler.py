...
class BlockRewriteSamplerMixin:
    def _build_default_block_writes_map(
        self,
        sample_output: SampleOutputBase,
    ) -> dict[str, dict[str, dict[int, int]]]:
        with record_function("diffulex.sampler.block_rewrite.build_default"):
            block_writes_map: dict[str, dict[str, dict[int, int]]] = {}
            for req_id_str, accepted_blocks in sample_output.accepted_ids_map.items():
                with record_function("diffulex.sampler.block_rewrite.req"):
                    ...
                    for block_id_str, accepted_ids in accepted_blocks.items():
                        with record_function("diffulex.sampler.block_rewrite.block"):
                            if not accepted_ids:
                                continue
                            ...
                            if block_writes:
                                req_block_writes[block_id_str] = block_writes
    ...
    def _build_block_writes_map(
        ...
        **kwargs,
        ...
        return self._build_default_block_writes_map(sample_output)
    ...
    def _postprocess_sample_output(
        ...
        **kwargs,
        ...
        with record_function("diffulex.sampler.block_rewrite.super_postprocess"):
        ...
        with record_function("diffulex.sampler.block_rewrite.build_writes_map"):
            sample_output.block_writes_map = self._build_block_writes_map(
                reqs=reqs,
                split_logits=split_logits,
                temperatures=temperatures,
                sample_output=sample_output,
